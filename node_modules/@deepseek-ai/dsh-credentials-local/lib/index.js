...
function parseCredentialsDocument(text, filename) {
	...
	if (fields["version"] === "1" && !("refs" in fields) && !("records" in fields)) {
		const refs = /* @__PURE__ */ new Map();
		for (const key of keys) {
			if (key === "version") continue;
			if (typeof fields[key] !== "string" || fields[key].length === 0) continue;
			refs.set(key, fields[key]);
		}
		return { refs, records: /* @__PURE__ */ new Map() };
	}
