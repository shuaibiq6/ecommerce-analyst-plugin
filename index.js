  ...
  "src/platform/rest.ts"() {
    ...
    RestAdapter = class {
      ...
      async updateProduct(sku, patch) {
        const data = await this.request(`/products/${sku}`, {
          method: "PATCH",
      ...
      async deleteProduct(sku) {
        await this.request(`/products/${sku}`, { method: "DELETE" });
      }
...
var EcommerceStore = class {
  ...
  async updateProduct(sku, patch) {
    ...
    if (patch.price !== void 0) {
      patch.price = fromCents(toCents(patch.price));
    }
    Object.assign(product, patch, { updated_at: (/* @__PURE__ */ new Date()).toISOString() });
    this.save();
    return product;
  }
  async deleteProduct(sku) {
  ...
  async setProductStatus(sku, status) {
    const product = this.getProduct(sku);
    if (!product) throw new Error(`\u5546\u54C1\u4E0D\u5B58\u5728\uFF1A${sku}`);
...
function registerProductTools(ctx, store) {
  ...
  ctx.tools.register(defineTool({
    ...
    parameters: {
      ...
      price: { type: "number", required: true, description: "\u552E\u4EF7\uFF08\xA5\uFF09\uFF0C\u5927\u4E8E 0" },
    ...
    output: {
      ...
      render: (_args, value) => {
        const v = value;
        return [{ type: "text", text: `\u5DF2\u65B0\u589E\u5546\u54C1 ${v.sku}\u300C${v.name}\u300D\uFF08\xA5${v.price.toFixed(2)}\uFF0C\u5E93\u5B58 ${v.stock}\uFF09` }];
      }
    },
    async execute(args) {
  ...
  ctx.tools.register(defineTool({
    ...
    parameters: {
      ...
      price: { type: "number", description: "\u65B0\u7684\u552E\u4EF7\uFF08\xA5\uFF09" },
    ...
    output: {
      ...
      render: (_args, value) => {
        ...
        return [{
          type: "text",
          text: `\u5DF2\u66F4\u65B0\u5546\u54C1 ${v.sku}\u300C${v.name}\u300D\uFF08\xA5${v.price.toFixed(2)}\uFF0C\u5E93\u5B58 ${v.stock}\uFF0C${v.status === "on_sale" ? "\u5728\u552E" : "\u4E0B\u67B6"}\uFF09`
        }];
      }
    },
    async execute(args) {
      if (args.price !== void 0 && args.price <= 0) throw new Error("\u552E\u4EF7\u5FC5\u987B\u5927\u4E8E 0");
      if (args.stock !== void 0 && (!Number.isInteger(args.stock) || args.stock < 0)) throw new Error("\u5E93\u5B58\u5FC5\u987B\u662F\u975E\u8D1F\u6574\u6570");
...
function renderDataCenter(_store) {
  try {
    return readFileSync2(join(MODULE_DIR, "assets", "data-center.html"), "utf8");
  } catch (err) {
...
function injectApiBase(webServer) {
  ...
  return webServer.tapIndex((html) => {
    const tag = base ? `<script>window.__ECOM_API_BASE__ = ${JSON.stringify(base)};</script>` : "<script>window.__ECOM_API_BASE__ = null;</script>";
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        if (pathname === "/ecommerce-api/products/all" && req.method === "GET") {
          const items = store.listProducts({ page_size: 1e4 }).items;
        ...
        if (pathname === "/ecommerce-api/product/update" && req.method === "POST") {
          ...
          try {
            ...
            if (body.category !== void 0) patch.category = String(body.category);
